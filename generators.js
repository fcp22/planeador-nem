// ============================================================
// GENERADORES DE PROMPTS — PLANEADOR NEM
// Genera Programa Analítico y Plano Didáctico
// Formato HTML con tablas — idéntico a Mtra. Yolanda FCP
// ============================================================

// ── GROQ CONFIG ───────────────────────────────────────────
const GROQ_KEY = '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';
const GROQ_URL = 'https://api.groq.com/openai/v1/chat/completions';
const GROQ_MODEL = 'llama-3.3-70b-versatile';

async function llamarGemini(prompt) {
  let intentos = 0;
  const maxIntentos = 3;
  while (intentos < maxIntentos) {
    try {
      const res = await fetch(GROQ_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${GROQ_KEY}`
        },
        body: JSON.stringify({
          model: GROQ_MODEL,
          messages: [{ role: 'user', content: prompt }],
          temperature: 0.7,
          max_tokens: 8192
        })
      });
      if (res.status === 429) {
        intentos++;
        if (intentos < maxIntentos) { await new Promise(r => setTimeout(r, 3000 * intentos)); continue; }
        throw new Error('El servicio está ocupado. Por favor espera unos segundos e intenta de nuevo.');
      }
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error?.message || `Error HTTP ${res.status}`);
      }
      const data = await res.json();
      const texto = data.choices?.[0]?.message?.content;
      if (!texto) throw new Error('Respuesta vacía del servicio de IA');
      return texto;
    } catch (e) {
      if (intentos >= maxIntentos - 1) throw e;
      intentos++;
      await new Promise(r => setTimeout(r, 2000));
    }
  }
}


// ============================================================
// GENERADOR 1: PROGRAMA ANALÍTICO — SALIDA EN HTML CON TABLAS
// ============================================================
function buildPromptAnalitico(d) {
  const contenidosTexto = (arr) => arr.map(c => `• ${c.titulo}`).join('\n');
  const pdasTexto = (arr, grado) => arr.map(c => `• ${obtenerPDA(c, grado)}`).join('\n');

  const t = (n) => n === '1' ? 'PRIMER' : n === '2' ? 'SEGUNDO' : 'TERCER';

  return `Eres un experto en la Nueva Escuela Mexicana (NEM) de México.
Genera un PROGRAMA ANALÍTICO COMPLETO para docentes de secundaria.

IMPORTANTE: Tu respuesta debe ser EXCLUSIVAMENTE código HTML listo para insertar en un documento Word.
NO incluyas etiquetas <html>, <head>, <body> ni <!DOCTYPE>.
NO incluyas explicaciones, comentarios ni texto fuera del HTML.
USA ÚNICAMENTE tablas HTML con el estilo exacto indicado.

DATOS DEL DOCENTE:
- Escuela: ${d.escuela}
- CCT: ${d.cct || '31DES0008B'}
- Turno: ${d.turno || 'Matutino'}
- Campo formativo: ${d.campo}
- Asignatura/Disciplina: ${d.disciplina}
- Grado: ${d.grado}° de Secundaria
- Docente(s): ${d.docente}
- Ciclo escolar: ${d.cicloEscolar || '2025-2026'}
- Municipio: ${d.municipio || 'Ticul, Yucatán'}

TRIMESTRE 1 - Contenidos: ${contenidosTexto(d.t1?.contenidos || [])}
TRIMESTRE 1 - PDAs: ${pdasTexto(d.t1?.contenidos || [], d.grado)}
TRIMESTRE 1 - Proyecto: ${d.t1?.nombreProyecto || ''}
TRIMESTRE 1 - Sesiones: ${d.t1?.sesiones || ''}
TRIMESTRE 1 - Problematización: ${d.t1?.problematizacion || ''}

TRIMESTRE 2 - Contenidos: ${contenidosTexto(d.t2?.contenidos || [])}
TRIMESTRE 2 - PDAs: ${pdasTexto(d.t2?.contenidos || [], d.grado)}
TRIMESTRE 2 - Proyecto: ${d.t2?.nombreProyecto || ''}
TRIMESTRE 2 - Sesiones: ${d.t2?.sesiones || ''}

TRIMESTRE 3 - Contenidos: ${contenidosTexto(d.t3?.contenidos || [])}
TRIMESTRE 3 - PDAs: ${pdasTexto(d.t3?.contenidos || [], d.grado)}
TRIMESTRE 3 - Proyecto: ${d.t3?.nombreProyecto || ''}
TRIMESTRE 3 - Sesiones: ${d.t3?.sesiones || ''}

GENERA EL SIGUIENTE HTML EXACTAMENTE CON ESTA ESTRUCTURA:

<h2 style="text-align:center;font-size:14pt;font-weight:bold;">Programa Analítico</h2>
<h3 style="text-align:center;">${d.grado}° grado</h3>

<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;width:35%;font-weight:bold;background:#BDD7EE;">Escuela:</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;">${d.escuela}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Turno/horario:</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;">${d.turno || 'Matutino'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Campo formativo:</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;">${d.campo}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Asignatura/Disciplina:</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;">${d.disciplina}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Grado:</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;">${d.grado}°</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Problemática/Situación de aprendizaje:</td>
    <td style="border:1px solid #000;padding:5px 8px;">[GENERA: situación de aprendizaje central del ciclo relacionada con ${d.disciplina}]</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Metodología:</td>
    <td style="border:1px solid #000;padding:5px 8px;">ABP</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Curso escolar:</td>
    <td style="border:1px solid #000;padding:5px 8px;">${d.cicloEscolar || '2025-2026'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">Docentes de ${d.disciplina}</td>
    [SI HAY DISCIPLINAS ADICIONALES AGREGA MÁS COLUMNAS AQUÍ]
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;">${d.docente}</td>
  </tr>
</table>

<p style="font-weight:bold;font-size:12pt;">PLANO UNO: Análisis del contexto socioeducativo de la Escuela</p>

<p style="font-weight:bold;">Contexto Socioeconómico:</p>

<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      <p><strong>Datos de identificación:</strong></p>
      <p>[GENERA: descripción de identificación de la escuela ${d.escuela}, CCT ${d.cct || '31DES0008B'}, ubicada en ${d.municipio || 'Ticul, Yucatán'}, con datos de matrícula y plantilla docente típicos de secundaria general en Yucatán]</p>
      <p><strong>Descripción del contexto escolar:</strong></p>
      <p>[GENERA: características del centro escolar, horarios, infraestructura, plantilla, ambiente de trabajo, comunicación entre academias]</p>
      <p><strong>Características del contexto familiar, sociocultural y lingüístico:</strong></p>
      <p>[GENERA: contexto socioeconómico, actividades económicas de la comunidad, nivel socioeconómico, lengua materna, situaciones de riesgo social]</p>
    </td>
  </tr>
</table>

<p><strong>Características y necesidades de los alumnos del centro escolar:</strong></p>
<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      [GENERA: datos de reprobación, rezago, deserción, causas, NEE, principales barreras para el aprendizaje en contexto de secundaria en Yucatán]
    </td>
  </tr>
</table>

<p><strong>Características de los ambientes de aprendizaje:</strong></p>
<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      [GENERA: estrategias de atención a estilos de aprendizaje, uso de tecnología, actividades compensatorias, estrategias específicas para ${d.disciplina}]
    </td>
  </tr>
</table>

<p style="font-weight:bold;">Resultados académicos:</p>
<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      [GENERA: resultados de evaluaciones internas y externas (IDAESY u otras) con porcentajes por nivel de logro, áreas de oportunidad en ${d.disciplina} para ${d.grado}° de secundaria]
    </td>
  </tr>
</table>

<p style="font-weight:bold;">Problemática detectada y jerarquizada:</p>
<table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      ${d.problematica || '[GENERA: 2 problemáticas relevantes detectadas, relacionadas con ' + d.disciplina + ', con descripción clara del reto y la causa, redactadas en lenguaje NEM humanista y comunitario]'}
    </td>
  </tr>
</table>

<p style="font-weight:bold;font-size:12pt;">SEGUNDO PLANO: CONTEXTUALIZACIÓN.</p>

<!-- ══ PRIMER TRIMESTRE ══ -->
<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;width:35%;">Periodo de evaluación</td>
    <td style="border:1px solid #000;padding:5px 8px;">Primer trimestre</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Mes/año:</td>
    <td style="border:1px solid #000;padding:5px 8px;">Septiembre - Noviembre &nbsp;&nbsp; Curso escolar: ${d.cicloEscolar || '2025-2026'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Número de sesiones</td>
    <td style="border:1px solid #000;padding:5px 8px;">${d.t1?.sesiones || '[sesiones]'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Modalidad didáctica: ${d.t1?.modalidad || 'Proyectos comunitarios'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Tipo de proyecto:</strong> ${d.t1?.tipoProyecto || 'ABP'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Nombre del proyecto:</strong> ${d.t1?.nombreProyecto || '[GENERA: nombre de proyecto pertinente para los contenidos del primer trimestre de ' + d.disciplina + ']'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Problematización de la realidad:</strong><br>${d.t1?.problematizacion || '[GENERA: problematización de la realidad pertinente al contexto escolar y los contenidos del 1er trimestre]'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Propósitos del proyecto:</strong><br>${d.t1?.proposito || '[GENERA: propósitos claros y medibles alineados a la NEM para el 1er trimestre]'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">CONTENIDOS</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      <strong>${d.disciplina}:</strong><br>
      ${contenidosTexto(d.t1?.contenidos || [])}
    </td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">PDA</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      ${pdasTexto(d.t1?.contenidos || [], d.grado)}
    </td>
  </tr>
</table>

<p style="font-weight:bold;">ELEMENTOS QUE SE DESARROLLAN CON EL CONTENIDO ABORDADO:</p>

<p style="font-weight:bold;">PERFIL DE EGRESO:</p>
<p>[GENERA: 3-4 elementos del perfil de egreso NEM pertinentes a los contenidos del 1er trimestre de ${d.disciplina}, redactados completos]</p>

<p style="font-weight:bold;">EJES ARTICULADORES:</p>
<p>${d.t1?.ejes || 'Inclusión, Pensamiento crítico, Interculturalidad crítica, Igualdad de género, Vida saludable, Apropiación de las culturas a través de la lectura y la escritura, Artes y experiencias estéticas'}</p>

<p style="font-weight:bold;">FINALIDADES DEL CAMPO FORMATIVO:</p>
<p>[GENERA: 2-3 finalidades del campo formativo ${d.campo} relacionadas con los contenidos del 1er trimestre]</p>

<p style="font-weight:bold;">ESPECIFICIDAD DEL CAMPO FORMATIVO:</p>
<p>[GENERA: especificidad del campo formativo ${d.campo} para ${d.disciplina} en 1er trimestre]</p>

<table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Productos o entregables/evidencias ${d.grado}° grado. PRIMER TRIMESTRE</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      <strong>${d.disciplina}:</strong> ${d.t1?.productos || '[GENERA: 1-2 productos concretos y evaluables para el 1er trimestre]'}
    </td>
  </tr>
</table>

<!-- ══ SEGUNDO TRIMESTRE ══ -->
<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;width:35%;">Periodo de evaluación</td>
    <td style="border:1px solid #000;padding:5px 8px;">Segundo trimestre</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Mes/año:</td>
    <td style="border:1px solid #000;padding:5px 8px;">Diciembre - Marzo &nbsp;&nbsp; Curso escolar: ${d.cicloEscolar || '2025-2026'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Número de sesiones</td>
    <td style="border:1px solid #000;padding:5px 8px;">${d.t2?.sesiones || '[sesiones]'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Modalidad didáctica: ${d.t2?.modalidad || 'Proyectos comunitarios'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Tipo de proyecto:</strong> ${d.t2?.tipoProyecto || 'ABP'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Nombre del proyecto:</strong> ${d.t2?.nombreProyecto || '[GENERA: nombre de proyecto para el 2do trimestre de ' + d.disciplina + ']'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Problematización de la realidad:</strong><br>${d.t2?.problematizacion || '[GENERA: problematización pertinente para el 2do trimestre]'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Propósitos del proyecto:</strong><br>${d.t2?.proposito || '[GENERA: propósitos alineados a la NEM para el 2do trimestre]'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">CONTENIDOS</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      <strong>${d.disciplina}:</strong><br>
      ${contenidosTexto(d.t2?.contenidos || [])}
    </td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">PDA</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      ${pdasTexto(d.t2?.contenidos || [], d.grado)}
    </td>
  </tr>
</table>

<p style="font-weight:bold;">PERFIL DE EGRESO:</p>
<p>[GENERA: 3-4 elementos del perfil de egreso NEM para 2do trimestre de ${d.disciplina}]</p>

<p style="font-weight:bold;">EJES ARTICULADORES:</p>
<p>${d.t2?.ejes || 'Inclusión, Igualdad de género, Apropiación de las culturas a través de la lectura y la escritura, Artes y experiencias estéticas'}</p>

<p style="font-weight:bold;">FINALIDADES DEL CAMPO FORMATIVO:</p>
<p>[GENERA: 2-3 finalidades del campo ${d.campo} para 2do trimestre]</p>

<p style="font-weight:bold;">ESPECIFICIDAD DEL CAMPO FORMATIVO:</p>
<p>[GENERA: especificidad para 2do trimestre en ${d.disciplina}]</p>

<table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Productos o entregables/evidencias ${d.grado}° grado. SEGUNDO TRIMESTRE</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      <strong>${d.disciplina}:</strong> ${d.t2?.productos || '[GENERA: productos concretos para el 2do trimestre]'}
    </td>
  </tr>
</table>

<!-- ══ TERCER TRIMESTRE ══ -->
<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;width:35%;">Periodo de evaluación</td>
    <td style="border:1px solid #000;padding:5px 8px;">Tercer trimestre</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Mes/año:</td>
    <td style="border:1px solid #000;padding:5px 8px;">Abril - Junio 2026 &nbsp;&nbsp; Curso escolar: ${d.cicloEscolar || '2025-2026'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Número de sesiones</td>
    <td style="border:1px solid #000;padding:5px 8px;">${d.t3?.sesiones || '[sesiones]'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Modalidad didáctica: ${d.t3?.modalidad || 'Proyectos comunitarios'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Tipo de proyecto:</strong> ${d.t3?.tipoProyecto || 'ABP'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Nombre del proyecto:</strong> ${d.t3?.nombreProyecto || '[GENERA: nombre de proyecto para el 3er trimestre de ' + d.disciplina + ']'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Problematización de la realidad:</strong><br>${d.t3?.problematizacion || '[GENERA: problematización para el 3er trimestre]'}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"><strong>Propósitos del proyecto:</strong><br>${d.t3?.proposito || '[GENERA: propósitos NEM para el 3er trimestre]'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:8px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">CONTENIDOS</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      <strong>${d.disciplina}:</strong><br>
      ${contenidosTexto(d.t3?.contenidos || [])}
    </td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">PDA</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      ${pdasTexto(d.t3?.contenidos || [], d.grado)}
    </td>
  </tr>
</table>

<p style="font-weight:bold;">PERFIL DE EGRESO:</p>
<p>[GENERA: 3-4 elementos del perfil de egreso NEM para 3er trimestre de ${d.disciplina}]</p>

<p style="font-weight:bold;">EJES ARTICULADORES:</p>
<p>${d.t3?.ejes || 'Inclusión, Vida saludable, Pensamiento crítico, Igualdad de género'}</p>

<p style="font-weight:bold;">FINALIDADES DEL CAMPO FORMATIVO:</p>
<p>[GENERA: 2-3 finalidades del campo ${d.campo} para 3er trimestre]</p>

<p style="font-weight:bold;">ESPECIFICIDAD DEL CAMPO FORMATIVO:</p>
<p>[GENERA: especificidad para 3er trimestre en ${d.disciplina}]</p>

<table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Productos o entregables/evidencias ${d.grado}° grado. TERCER TRIMESTRE</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:6px 10px;">
      <strong>${d.disciplina}:</strong> ${d.t3?.productos || '[GENERA: productos concretos para el 3er trimestre]'}
    </td>
  </tr>
</table>

INSTRUCCIÓN FINAL CRÍTICA:
- Sustituye TODOS los textos entre corchetes [GENERA: ...] con contenido pedagógico real, específico y aplicable.
- Respeta TODO el HTML tal como está: no cambies los estilos, la estructura de tablas ni las etiquetas.
- NO agregues texto fuera de las etiquetas HTML.
- Usa lenguaje NEM: humanista, comunitario, inclusivo y orientado a proyectos.
- El documento debe verse idéntico al formato oficial de la Secundaria Felipe Carrillo Puerto.`;
}

// ============================================================
// GENERADOR 2: PLANO DIDÁCTICO — SALIDA EN HTML CON TABLAS
// ============================================================
function buildPromptPlanDidacticoTrimestre(d) {
  const contenidosRows = d.contenidos.map(c =>
    `<tr>
      <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;">Tu Disciplina — ${d.disciplina}</td>
      <td style="border:1px solid #000;padding:5px 8px;">${c.titulo}</td>
      <td style="border:1px solid #000;padding:5px 8px;">${obtenerPDA(c, d.grado)}</td>
      <td style="border:1px solid #000;padding:5px 8px;">[GENERA: producto parcial o final concreto para este PDA en ${d.disciplina}]</td>
    </tr>`
  ).join('\n');

  const trimestreNombre = d.trimestre === '1' ? 'PRIMER' : d.trimestre === '2' ? 'SEGUNDO' : 'TERCER';

  return `Eres un experto en la Nueva Escuela Mexicana (NEM) de México.
Genera un PLANO DIDÁCTICO COMPLETO para el ${trimestreNombre} TRIMESTRE.

IMPORTANTE: Tu respuesta debe ser EXCLUSIVAMENTE código HTML.
NO incluyas etiquetas <html>, <head>, <body> ni <!DOCTYPE>.
NO incluyas explicaciones ni texto fuera del HTML.
USA ÚNICAMENTE tablas HTML con los estilos indicados.

DATOS:
- Escuela: ${d.escuela}
- CCT: ${d.cct || '31DES0008B'}
- Campo: ${d.campo}
- Disciplina: ${d.disciplina}
- Grado y grupo: ${d.grado}° ${d.grupo || 'A, B, C'}
- Docente: ${d.docente}
- Periodo: ${trimestreNombre} TRIMESTRE
- Modalidad: ${d.modalidad || 'Aprendizaje en proyectos comunitarios'}
- Nombre del proyecto: ${d.nombreProyecto}
- Problematización: ${d.problematizacion}
- Propósito: ${d.proposito}
- Sesiones: ${d.numSesiones || '11'} de ${d.duracionSesion || '50'} minutos
- F1 sesiones: ${d.sesionesF1 || '3'}
- F2 sesiones: ${d.sesionesF2 || '4'}
- F3 sesiones: ${d.sesionesF3 || '4'}

GENERA EL SIGUIENTE HTML:

<h2 style="text-align:center;font-weight:bold;font-size:13pt;">Plano didáctico</h2>

<p style="font-weight:bold;">ESCUELA SECUNDARIA GENERAL: "${d.escuela}", CLAVE ${d.cct || '31DES0008B'}, CICLO ESCOLAR ${d.cicloEscolar || '2025-2026'}</p>
<p><strong>CAMPO FORMATIVO:</strong> ${d.campo}</p>
<p><strong>DISCIPLINA:</strong> ${d.disciplina}</p>
<p><strong>GRADO Y GRUPO.</strong> ${d.grado}° ${d.grupo || 'A, B, C'}</p>
<p><strong>DOCENTE(S):</strong> ${d.docente}</p>
<p><strong>PERIODO:</strong> ${trimestreNombre} TRIMESTRE.</p>
<p><strong>MODALIDAD Y/O METODOLOGIA:</strong> ${d.modalidad || 'Aprendizaje en proyectos comunitarios'}</p>
<p><strong>NOMBRE DEL PROYECTO:</strong> ${d.nombreProyecto}</p>

<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;width:35%;">Problematización de la realidad o tema de interés</td>
    <td style="border:1px solid #000;padding:5px 8px;">${d.problematizacion}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Propósito u Objetivo</td>
    <td style="border:1px solid #000;padding:5px 8px;">${d.proposito}</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Perfil de egreso al cual contribuye</td>
    <td style="border:1px solid #000;padding:5px 8px;">
      [GENERA: lista de 8-10 rasgos del perfil de egreso NEM pertinentes a ${d.disciplina}, redactados completos tal como aparecen en el programa oficial]
    </td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Finalidades del campo formativo (programa sintético pág. 29 a 114)</td>
    <td style="border:1px solid #000;padding:5px 8px;">[GENERA: finalidades del campo formativo ${d.campo} para ${d.disciplina}, 2-3 oraciones]</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Especificidades del campo formativo</td>
    <td style="border:1px solid #000;padding:5px 8px;">[GENERA: especificidad del campo ${d.campo} aplicada a ${d.disciplina}, 2 oraciones]</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;">Ejes Articuladores</td>
    <td style="border:1px solid #000;padding:5px 8px;">Inclusión &nbsp;|&nbsp; Pensamiento crítico &nbsp;|&nbsp; Interculturalidad crítica &nbsp;|&nbsp; Igualdad de género &nbsp;|&nbsp; Vida saludable &nbsp;|&nbsp; Apropiación de las culturas a través de la lectura y la escritura &nbsp;|&nbsp; Artes y Experiencias estéticas</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">Interdisciplinariedad</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">CONTENIDOS</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">PDAS</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">PRODUCTO parcial o final</td>
  </tr>
  ${contenidosRows}
  <tr>
    <td colspan="4" style="border:1px solid #000;padding:5px 8px;font-weight:bold;text-align:center;">PRODUCTO INTEGRADOR: ${d.productoIntegrador || '[GENERA: producto integrador que articule todos los contenidos y PDAs]'}</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;width:20%;">FASES MOMENTOS ETAPAS (En caso de trabajar proyectos)</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;width:10%;">TEMPORALIDAD SESIONES</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;width:40%;">Actividades (del docente y del alumno)</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;width:15%;">Materiales y recursos didácticos (del docente y del alumno)</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;width:15%;">Evaluación formativa (Evidencias de aprendizaje)</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      <strong>F1. Planeación</strong> (Se identifica un problema y se negocia una ruta de acción)<br>
      Momento 1. Identificación<br>
      Momento 2. Recuperación<br>
      Momento 3. Planificación
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;text-align:center;"><strong>${d.sesionesF1 || '3'} sesiones</strong></td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      [GENERA: descripción detallada de ${d.sesionesF1 || '3'} clases de F1 para ${d.disciplina}, cada una con:
      <strong>Clase #N</strong><br>
      <strong>Inicio: (10 minutos)</strong> descripción de actividad de inicio...<br>
      <strong>Desarrollo: (30 minutos)</strong> descripción de actividad de desarrollo...<br>
      <strong>Cierre: (10 minutos)</strong> descripción del cierre y producto parcial...<br>
      Usa actividades reales, concretas y alineadas al proyecto "${d.nombreProyecto}" de ${d.disciplina} ${d.grado}°]
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">[GENERA: lista de materiales para F1]</td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">[GENERA: evidencias de evaluación formativa para F1]</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      <strong>F2. Acción</strong> (Se realizan producciones que permiten atender el problema)<br>
      Momento 4. Acercamiento<br>
      Momento 5. Comprensión y producción<br>
      Momento 6. Reconocimiento<br>
      Momento 7. Concreción
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;text-align:center;"><strong>${d.sesionesF2 || '4'} sesiones</strong></td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      [GENERA: descripción detallada de ${d.sesionesF2 || '4'} clases de F2, usando el mismo formato Inicio/Desarrollo/Cierre. Incluye trabajo en equipo, producción de materiales, aplicación de conocimientos de ${d.disciplina} en el proyecto "${d.nombreProyecto}"]
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">[GENERA: materiales para F2]</td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">[GENERA: evidencias F2]</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      <strong>F3. Intervención</strong> (Se difunden producciones, se da seguimiento y se plantea cómo mejorar)<br>
      Momento 8. Integración<br>
      Momento 9. Difusión<br>
      Momento 10. Consideraciones<br>
      Momento 11. Avances
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;text-align:center;"><strong>${d.sesionesF3 || '4'} sesiones</strong></td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      [GENERA: descripción detallada de ${d.sesionesF3 || '4'} clases de F3, enfocadas en presentación, difusión comunitaria, coevaluación y retroalimentación del producto integrador "${d.productoIntegrador || 'producto final'}"]
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">[GENERA: materiales para F3]</td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">[GENERA: evidencias F3]</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:10px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;width:30%;">Observaciones y/o Ajustes razonables</td>
    <td style="border:1px solid #000;padding:5px 8px;">[GENERA: observaciones pertinentes sobre ajustes posibles según las necesidades de los alumnos, el contexto y las fechas del trimestre]</td>
  </tr>
</table>

<table style="width:100%;border-collapse:collapse;margin-bottom:6px;">
  <tr>
    <td colspan="5" style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">EVALUACIÓN SUMATIVA</td>
  </tr>
  <tr>
    <td colspan="5" style="border:1px solid #000;padding:5px 8px;font-weight:bold;text-align:center;">CRITERIOS DE EVALUACIÓN Y PONDERACIÓN</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#E2EFDA;text-align:center;">Proceso (60) %</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#E2EFDA;text-align:center;">Modalidad</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#E2EFDA;text-align:center;">Técnica/Instrumento</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#E2EFDA;text-align:center;">Producto (40) %</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#E2EFDA;text-align:center;">Técnica/Instrumento</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      [GENERA: lista de actividades evaluativas enumeradas del 1 al 8-10, con porcentajes que sumen 60%: actividades en clase, participaciones, investigaciones, libreta, portafolio, examen parcial]
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      El maestro<br>El alumno<br>Autoevaluación<br>Coevaluación<br>Heteroevaluación
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      Observación.<br>Desempeño del alumno<br>Análisis del desempeño.<br>Interrogatorio.
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      [GENERA: 2 productos con porcentajes que sumen 40%: evidencia entregable y examen final]
    </td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">
      [GENERA: técnica e instrumento de evaluación para el producto final, como lista de cotejo, rúbrica o prueba escrita]
    </td>
  </tr>
</table>

<p style="font-weight:bold;">Guía de Evaluación.</p>
<table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">Agente y Modalidades</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">Técnicas</td>
    <td style="border:1px solid #000;padding:5px 8px;font-weight:bold;background:#BDD7EE;text-align:center;">Instrumentos de evaluación</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">El maestro<br>El alumno<br>Autoevaluación<br>Coevaluación<br>Heteroevaluación</td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">Observación.</td>
    <td style="border:1px solid #000;padding:5px 8px;vertical-align:top;">- Guía de observación<br>- Escala de actitudes</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"></td>
    <td style="border:1px solid #000;padding:5px 8px;">Desempeño del alumno.</td>
    <td style="border:1px solid #000;padding:5px 8px;">Preguntas sobre procedimiento.<br>Cuadernos de los alumnos<br>Organizadores gráficos</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"></td>
    <td style="border:1px solid #000;padding:5px 8px;">Análisis del desempeño.</td>
    <td style="border:1px solid #000;padding:5px 8px;">- Escala estimativa<br>- Lista de cotejo</td>
  </tr>
  <tr>
    <td style="border:1px solid #000;padding:5px 8px;"></td>
    <td style="border:1px solid #000;padding:5px 8px;">Interrogatorio</td>
    <td style="border:1px solid #000;padding:5px 8px;">- Tipos orales o escritos: Pruebas escritas.<br>- [GENERA: tipos textuales específicos del producto de ${d.disciplina}]</td>
  </tr>
</table>

INSTRUCCIÓN FINAL CRÍTICA:
- Sustituye TODOS los textos entre corchetes [GENERA: ...] con contenido pedagógico real y específico para ${d.disciplina} ${d.grado}°.
- Conserva TODA la estructura HTML y los estilos exactamente como están.
- Las actividades de cada fase deben ser detalladas, concretas, con inicio/desarrollo/cierre y usando lenguaje NEM.
- NO agregues texto fuera de las etiquetas HTML.`;
}

// ============================================================
// GENERADOR 3: PLANO DIDÁCTICO SEMANAL (mantener funcionalidad)
// ============================================================
function buildPromptPlanDidacticoSemanal(d, semana, sesionesEnSemana) {
  const contenidosTexto = d.contenidos.map(c =>
    `• Contenido: ${c.titulo}\n  PDA: ${obtenerPDA(c, d.grado)}`
  ).join('\n\n');

  return `Eres un experto en la Nueva Escuela Mexicana (NEM). Genera actividades semanales detalladas para una clase de ${d.disciplina} de ${d.grado}° de secundaria.

Semana: ${semana}
Sesiones esta semana: ${sesionesEnSemana.length}
Proyecto: ${d.nombreProyecto}
Disciplina: ${d.disciplina}
Grado: ${d.grado}°

Contenidos y PDAs:
${contenidosTexto}

Para cada sesión genera:
CLASE #[N] (${sesionesEnSemana[0] || 'Sesión'})
INICIO: ([X] minutos)
[Actividad de inicio: bienvenida, recuperación de saberes previos, motivación]

DESARROLLO: ([X] minutos)
[Actividad principal: explicación, práctica, trabajo individual o en equipo]

CIERRE: ([X] minutos)
[Síntesis, producto parcial, reflexión o tarea]

INSTRUCCIONES: Genera actividades reales, concretas y aplicables para ${d.disciplina} ${d.grado}° de secundaria. No uses corchetes en el resultado — rellena con contenido pedagógico específico, creativo y alineado a la NEM.`;
}

const LOGO_B64 = 'data:image/png;base64,';

// ============================================================
// DATOS INSTITUCIONALES FIJOS — NO MODIFICAR
// ============================================================
const INSTITUCION = {
  nombre: 'Escuela Secundaria General "Felipe Carrillo Puerto"',
  cct: '31DES0008B',
  municipio: 'Ticul, Yucatán',
  subsistema: 'Secundarias Generales — SEP',
  ciclo: '2025-2026',
  director: '',
  sello: '© Documento generado por el Planeador Docente NEM — Uso exclusivo FCP Ticul'
};

// ============================================================
// DESCARGA EN WORD — CONTENIDO HTML CON TABLAS
// CRÍTICO: NO SE ESCAPA EL HTML (sin .replace para &lt;)
// ============================================================
function descargarWord(htmlContenido, nombreArchivo) {
  const fecha = new Date().toLocaleDateString('es-MX', {
    weekday:'long', year:'numeric', month:'long', day:'numeric'
  });

  const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office"
    xmlns:w="urn:schemas-microsoft-com:office:word"
    xmlns="http://www.w3.org/TR/REC-html40">
<head>
<meta charset="UTF-8">
<style>
  body {
    font-family: Calibri, Arial, sans-serif;
    font-size: 11pt;
    margin: 1.5cm 2cm;
    line-height: 1.4;
    color: #1a1a2e;
  }
  table { border-collapse: collapse; width: 100%; margin: 6px 0; }
  td, th { border: 1px solid #000; padding: 5px 8px; font-size: 10.5pt; vertical-align: top; }
  p { margin: 4px 0; }
  h2, h3 { margin: 8px 0 4px 0; }
  .enc-sello {
    background: #f1f5f9;
    border-left: 4px solid #1a3a5c;
    padding: 4px 12px;
    font-size: 9pt;
    color: #334155;
    margin-bottom: 14px;
    font-style: italic;
  }
  .pie {
    margin-top: 24px;
    padding-top: 10px;
    border-top: 2px solid #1a3a5c;
    font-size: 9pt;
    color: #64748b;
    text-align: center;
  }
  @page { margin: 1.5cm 2cm; }
</style>
</head>
<body>

<table style="width:100%;border:none;border-bottom:3px solid #1a3a5c;margin-bottom:10px;padding-bottom:8px;">
  <tr>
    <td style="border:none;width:80px;vertical-align:middle;padding:0;">
      <img src="${LOGO_B64}" style="width:72px;height:auto;" alt="Escudo FCP">
    </td>
    <td style="border:none;vertical-align:middle;padding:0 0 0 12px;">
      <div style="font-size:12pt;font-weight:bold;color:#1a3a5c;">${INSTITUCION.nombre}</div>
      <div style="font-size:9.5pt;color:#334155;margin-top:3px;">
        CCT: <strong>${INSTITUCION.cct}</strong> &nbsp;·&nbsp;
        ${INSTITUCION.municipio} &nbsp;·&nbsp;
        ${INSTITUCION.subsistema}
      </div>
      <div style="font-size:9.5pt;color:#334155;">
        Ciclo escolar: <strong>${INSTITUCION.ciclo}</strong>
      </div>
    </td>
    <td style="border:none;width:80px;vertical-align:middle;text-align:right;padding:0;">
      <img src="${LOGO_B64}" style="width:72px;height:auto;" alt="Escudo FCP">
    </td>
  </tr>
</table>

<div class="enc-sello">
  ${INSTITUCION.sello} &nbsp;·&nbsp; Generado el ${fecha}
</div>

${htmlContenido}

<div class="pie">
  ${INSTITUCION.nombre} &nbsp;·&nbsp; CCT: ${INSTITUCION.cct} &nbsp;·&nbsp; ${INSTITUCION.municipio}<br>
  Documento generado con el Planeador Docente NEM — Programa Sintético Fase 6 — SEP 2025-2026<br>
  <em>Este documento es de uso exclusivo del personal docente de la institución.</em>
</div>

</body>
</html>`;

  const blob = new Blob([html], { type: 'application/msword' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = nombreArchivo;
  a.click();
  URL.revokeObjectURL(url);
}
